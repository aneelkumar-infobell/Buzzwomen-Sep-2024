...
func GetEnrollednagarikaGelathi(w http.ResponseWriter, r *http.Request, DB *sql.DB) {
	...
	if request.ProjectID != "0" && request.EmpID != "0" {
		...
		if len(request.StartDate) > 0 && len(request.EndDate) > 0 {
			search = fmt.Sprintf("AND date(tr_pat.nagarikaenrollmentdate) >= '%s' AND date(tr_pat.nagarikaenrollmentdate) <= '%s'", request.StartDate, request.EndDate)
		}
		...
		if roleID == 6 {
			...
			totalPagesSQL = fmt.Sprintf("SELECT COUNT(*) FROM training_participants tr_pat WHERE nagarikaenrolledproject in (%s) AND nagarikaenrollment = '1' and new_green!=1 and new_vyapar!=1 AND tb_id IN (%s) %s %s", strings.Join(projArray, ","), trainingBatchIds, search, searchFilter)
			fields = "tr_pat.id, tr_pat.firstName as gelathiname, SUBSTRING_INDEX(IFNULL(tbl_poa.name, ''), '_', 1) as villagename, tr_pat.tb_id, if((select id from gelathi_circle gelath_cr WHERE gelath_cr.gelathi_id = tr_pat.id LIMIT 1), 1, 0) as flag, IFNULL(DATE_FORMAT(tr_pat.nagarikaenrollmentdate, '%d-%m-%Y'), '') as nagarikaenrollmentdate, '' AS enrolled_by"
			query = fmt.Sprintf("SELECT %s FROM training_participants tr_pat LEFT JOIN tbl_poa ON tr_pat.tb_id = tbl_poa.id WHERE tr_pat.nagarikaenrolledproject in (%s) AND tr_pat.nagarikaenrollment = '1'  AND tr_pat.tb_id IN (%s) %s ORDER BY tr_pat.nagarikaenrollmentdate DESC", fields, strings.Join(projArray, ","), trainingBatchIds, searchFilter)
		} else {
			totalPagesSQL = fmt.Sprintf("SELECT COUNT(*) FROM training_participants tr_pat WHERE nagarikaenrolledproject in (%s) and new_green!=1 and new_vyapar!=1 AND nagarikaenrollment = 1 %s %s", strings.Join(projArray, ","), search, searchFilter)
			fieldsEnroll := "tr_pat.id, tr_pat.firstName as gelathiname, COALESCE(SUBSTRING_INDEX(tbl_poa.name, '_', 1),'') as villagename, '' as tb_id, '' as flag, IFNULL(DATE_FORMAT(tr_pat.nagarikaenrollmentdate, '%d-%m-%Y'), '') as nagarikaenrollmentdate, CONCAT(IFNULL(emp.first_name, ''),' ',IFNULL(emp.last_name, '')) AS enrolled_by"
			query = fmt.Sprintf("SELECT %s FROM training_participants tr_pat LEFT JOIN tbl_poa ON tr_pat.tb_id = tbl_poa.id LEFT JOIN employee emp ON tr_pat.gelathi_id = emp.id WHERE tr_pat.nagarikaenrolledproject in (%s)  AND tr_pat.nagarikaenrollment = '1' %s %s ORDER BY tr_pat.nagarikaenrollmentdate DESC", fieldsEnroll, strings.Join(projArray, ","), search, searchFilter)
		}
