...
type NagarikaProgramQuestionnaire struct {
	...
	DistrictTaluk                                          string   `json:"district"`
	IfTumkur                                               string   `json:"taluk"`
...
func Addnagarikasurvey(w http.ResponseWriter, r *http.Request, DB *sql.DB) {
	...
	`
		...
		_, err := DB.Exec(sql,
			...
			strings.Join(a.CanYouIdentifyTop3ProblemsInYourVillage, ","),
			...
			strings.Join(a.WhatDoYouDoInGramSabha, ","),
			...
			strings.Join(a.CanYouGoToThesePlacesWithoutPermissionFromFamilyMember, ","),
			strings.Join(a.CanYouGoToThesePlacesAlone, ","),
			strings.Join(a.WhatAreYourMostImpSourcesForInformationAboutGovernment, ","),
