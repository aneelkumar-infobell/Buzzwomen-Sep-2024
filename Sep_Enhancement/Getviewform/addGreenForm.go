...
func AddGreensurvey(w http.ResponseWriter, r *http.Request, DB *sql.DB) {
	...
	var survey GreenBaselineSurvey1
	err1 := json.NewDecoder(r.Body).Decode(&survey)
	...
	if survey.ParticipantID != 0 {
		...
		_, err := DB.Exec(sql,
			...
			survey.Email,
			survey.NameOfTheSurveyor,
			survey.NameOfTheRespondent,
			survey.VillageName,
			survey.PhoneNumber,
			survey.NaturalResources,
			survey.NaturalResourcesImpactingYourLife,
			survey.NaturalWealth,
			survey.ClimateChange,
			survey.WhatDoYouKnowAboutIt,
			survey.ChangeInTheWeatherClimate,
			survey.ChangesHappenedToTheClimate,
			survey.ClimateChangeThreatensHealthSafety,
			survey.DoneToTackleClimateChange,
			survey.DoSomethingToTackleClimateChange,
			survey.MainSourceOfWater,
			survey.AgreeWithStatements,
			survey.ConcernedLocalWaterQuality,
			survey.PersonalActionsAffectWaterQuality,
			survey.WaterConservationMeasures,
			survey.IfYesWhatKindOfMeasures,
			survey.ListDownImpactOfClimateChange,
			survey.GiftingLandToChildren,
			survey.WhichOneIsRight,
			survey.BelieveConnectionBetweenFoodHealth,
			survey.EnvironmentallyFriendlyNativeFood,
			survey.HouseholdPollutesNaturalResources,
			survey.AlternativesHouseholdMaterialsPollute,
			survey.IfYesWhatAreThey,
			survey.EcoFriendlyProductsActivities,
			survey.PayMoreForEcoFriendlyProducts,
			survey.SwitchToEcoFriendlyProducts,
			survey.ClimateChangeIsEffort,
			survey.ActionOutOfConcernForClimateChange,
			survey.IfYesWhatDidYouDo,
			survey.NaturalResourceAttentionMeasures,
			survey.IfYesWhatResource,
			survey.AchieveWithRegardToNaturalResourceConservation,
			...
			survey.Education,
			survey.PhoneType,
			survey.PrimaryOccupation,
			survey.SecondaryOccupation,
			survey.WomenOccupation,
			survey.HouseholdMigrationLastYear,
			survey.MigrantSendsRemittances,
			survey.HouseholdOwnsLand,
			survey.LandAcres,
			survey.MonthlyExpenditure,
			survey.MonthlyIncome,
			survey.DailyClimateAction,
			survey.RunsEnterprise,
			survey.EcoFriendlyPracticesEnterprise,
			survey.EcoFriendlyPracticesDetails,
			survey.WasteSegregationAtHome,
			survey.IsMenstruating,
			survey.MenstruationProductsUsed,
			survey.SanitaryDisposalMethod,
			survey.SoilChangesOverYears,
			survey.SoilObservationsIfNoChanges,
			survey.EssentialCharacteristicsFertileSoil,
			survey.MainCropGrown,
			survey.AccessToNutritionalFood,
			survey.ReasonsForLackOfNutritionalFood,
			survey.IdentifyTreesInCommunity,
			survey.TreeNamesInCommunity,
			survey.FoodProduction,
			survey.SellProduceLocalMarket,
			survey.UsePesticidesFertilizers,
			survey.CommunityGovernmentEnvironment,
			survey.ConserveLocalSeeds,
			survey.NaturalResourceConservationGoal,
			survey.Goal,
			survey.MobilizedCommunityForConservation,
			survey.GreenActionInCommunity,
			survey.DetailsOfGreenAction,
			survey.MainProductsServicesUsed,
			survey.HouseholdWasteManagement,
			survey.WasteCategoriesProduced,
			survey.AccessToDailyLivingProducts,
			survey.LocallyProducedProductsConsumed,
		)
