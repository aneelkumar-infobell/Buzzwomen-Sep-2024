...
func CreateGFSessionsnew2(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	stmt, err := db.Prepare("INSERT INTO tbl_poa (type, tb_id, name, user_id, project_id, location_id, participants, date, session_type, circle_id,funderid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)")
	if err != nil {
	...
	result, err := stmt.Exec(typ, request.TbID, name, request.UserID, request.ProjectID, request.LocationID, request.NumberOfParticipants, planDateWithoutSec, request.GfSessionType, request.CircleID, ffid)
	if err != nil {
