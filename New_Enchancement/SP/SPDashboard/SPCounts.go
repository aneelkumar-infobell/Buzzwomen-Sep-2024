...
func spoorthiEnrolled(db *sql.DB, startDate string, endDate string, projectArray []int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			spoorthienroll = fmt.Sprintf("SELECT COUNT(id) as spoorthi FROM training_participants WHERE enroll=1 and enroll_date BETWEEN '%s' and '%s' and project_id IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		} else {
			spoorthienroll = fmt.Sprintf("SELECT COUNT(id) as spoorthi FROM training_participants WHERE enroll=1 and project_id IN (%s) %s", strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		}
	} else {
		spoorthienroll = fmt.Sprintf("SELECT COUNT(id) as spoorthi FROM training_participants WHERE enroll=1 and enroll_date BETWEEN '%s' and '%s'", startDate, endDate)
	}
...
func spoorthiModule(db *sql.DB, startDate string, endDate string, projectArray []int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			spoorthimodule = fmt.Sprintf("SELECT count(module1=1 and module2=1 and module3=1 and module4=1 and module5=1) as noofspoorthimodules FROM SpoorthiBaselineQuestionnaire sb JOIN training_participants t ON t.id=sb.partcipantId where sb.entry_date BETWEEN '%s' and '%s' and t.project_id IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		} else {
			spoorthimodule = fmt.Sprintf("SELECT count(module1=1 and module2=1 and module3=1 and module4=1 and module5=1) as noofspoorthimodules FROM SpoorthiBaselineQuestionnaire sb JOIN training_participants t ON t.id=sb.partcipantId where t.project_id IN (%s) %s", strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		}
	} else {
		spoorthimodule = fmt.Sprintf("SELECT count(module1=1 and module2=1 and module3=1 and module4=1 and module5=1) as noofspoorthimodules FROM SpoorthiBaselineQuestionnaire sb JOIN training_participants t ON t.id=sb.partcipantId where sb.entry_date BETWEEN '%s' and '%s'", startDate, endDate)
	}
...
func spoorthiSurvey(db *sql.DB, startDate string, endDate string, projectArray []int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			spoorthisurvey = fmt.Sprintf("SELECT count(sb.id) as noofspoorthisurvey FROM SpoorthiBaselineQuestionnaire sb join training_participants t on t.id= sb.partcipantId where sb.entry_date BETWEEN '%s' and '%s' and t.project_id IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		} else {
			spoorthisurvey = fmt.Sprintf("SELECT count(sb.id) as noofspoorthisurvey FROM SpoorthiBaselineQuestionnaire sb join training_participants t on t.id= sb.partcipantId where t.project_id IN (%s) %s", strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		}
	} else {
		spoorthisurvey = fmt.Sprintf("SELECT count(sb.id) as noofspoorthisurvey FROM SpoorthiBaselineQuestionnaire sb join training_participants t on t.id= sb.partcipantId where sb.entry_date BETWEEN '%s' and '%s'", startDate, endDate)
	}
...
func noofCircleMeeting(db *sql.DB, startDate string, endDate string, projectArray []int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			noofcirclemeeting = fmt.Sprintf("SELECT COUNT(tbl_poa.session_type) AS noofspoorthicircle FROM tbl_poa tbl_poa WHERE tbl_poa.type = 2 AND (tbl_poa.session_type IN (4, 5, 6, 7, 8, 9)) and tbl_poa.date BETWEEN '%s' and '%s' and tbl_poa.project_id IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		} else {
			noofcirclemeeting = fmt.Sprintf("SELECT COUNT(tbl_poa.session_type) AS noofspoorthicircle FROM tbl_poa tbl_poa WHERE tbl_poa.type = 2 AND (tbl_poa.session_type IN (4, 5, 6, 7, 8, 9)) and tbl_poa.project_id IN (%s) %s", strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), filter)
		}
	} else {
		noofcirclemeeting = fmt.Sprintf("SELECT COUNT(tbl_poa.session_type) AS noofspoorthicircle FROM tbl_poa tbl_poa WHERE tbl_poa.type = 2 AND (tbl_poa.session_type IN (4, 5, 6, 7, 8, 9)) and tbl_poa.date BETWEEN '%s' and '%s'", startDate, endDate)
	}
...
func SpoorthinewVillageCount(con *sql.DB, startDate, endDate string, projectArray []int, filter string) (int, error) {
	...
	if len(projectArray) > 0 {
		villageQuery = fmt.Sprintf("select COUNT(DISTINCT location_id) from tbl_poa tp join training_participants t on t.project_id =tp.project_id where t.enroll = 1 and t.project_id IN (%s)", intArrayToString(projectArray))
		subVillageQuery = fmt.Sprintf("SELECT COUNT(DISTINCT sub_village) as 'subVillage' FROM tbl_poa tp join training_participants t on t.project_id =tp.project_id WHERE t.enroll = 1 and t.project_id IN (%s)", intArrayToString(projectArray))
		...
		villageQuery = fmt.Sprintf("select COUNT(DISTINCT location_id) from tbl_poa tp  INNER JOIN project p ON p.id = tp.project_id join training_participants t on t.project_id =tp.project_id where t.enroll = 1 AND p.startDate >= '%s' AND p.endDate <= '%s' %s", startDate, endDate, filter)
		subVillageQuery = fmt.Sprintf("SELECT COUNT(DISTINCT sub_village) as 'subVillage' FROM tbl_poa tp INNER JOIN project p ON p.id = tp.project_id join training_participants t on t.project_id =tp.project_id WHERE t.enroll = 1 AND p.startDate >= '%s' AND p.endDate <= '%s' %s", startDate, endDate, filter)
	}
...
func GfSpoorthiVillageCount(db *sql.DB, startDate string, endDate string, projectArray []int, user_id string) int {
	...
	if len(projectArray) > 0 {
		...
		subVillageQuery := fmt.Sprintf("SELECT COUNT(DISTINCT sub_village) as 'subVillage' FROM tbl_poa tp join training_participants t on t.project_id =tp.project_id WHERE t.enroll = 1 and t.project_id IN (%s) AND tp.user_id = %s", projectIDs, user_id)
...
func GflspoorthiEnrolled(db *sql.DB, startDate string, endDate string, projectArray []int, gflid int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			spoorthienroll = fmt.Sprintf("SELECT COUNT(id) as spoorthi FROM training_participants WHERE enroll=1 and enroll_date BETWEEN '%s' and '%s' and project_id IN (%s) and gelathi_id IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), strings.Trim(strings.Replace(fmt.Sprint(gfid), " ", ",", -1), "[]"), filter)
		} else {
			spoorthienroll = fmt.Sprintf("SELECT COUNT(id) as spoorthi FROM training_participants WHERE enroll=1 and project_id IN (%s) and gelathi_id IN (%s) %s", strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), strings.Trim(strings.Replace(fmt.Sprint(gfid), " ", ",", -1), "[]"), filter)
		}
	} else {
		spoorthienroll = fmt.Sprintf("SELECT COUNT(id) as spoorthi FROM training_participants WHERE enroll=1 and enroll_date BETWEEN '%s' and '%s'", startDate, endDate)
	}
...
func GflspoorthiModule(db *sql.DB, startDate string, endDate string, projectArray []int, gflid int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			spoorthimodule = fmt.Sprintf("SELECT count(module1=1 and module2=1 and module3=1 and module4=1 and module5=1) as noofspoorthimodules FROM SpoorthiBaselineQuestionnaire sb JOIN training_participants t ON t.id=sb.partcipantId where sb.entry_date BETWEEN '%s' and '%s' and t.project_id IN (%s) and sb.GelathiId IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), strings.Trim(strings.Replace(fmt.Sprint(gfid), " ", ",", -1), "[]"), filter)
		} else {
...
func GflnoofCircleMeeting(db *sql.DB, startDate string, endDate string, projectArray []int, gflid int, filter string) int {
	...
	if len(projectArray) > 0 {
		if startDate != "" && endDate != "" {
			noofcirclemeeting = fmt.Sprintf("SELECT COUNT(tbl_poa.session_type) AS noofspoorthicircle FROM tbl_poa tbl_poa WHERE tbl_poa.type = 2 AND (tbl_poa.session_type IN (4, 5, 6, 7, 8, 9)) and tbl_poa.date BETWEEN '%s' and '%s' and tbl_poa.project_id IN (%s) and tbl_poa.user_id IN (%s) %s", startDate, endDate, strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), strings.Trim(strings.Replace(fmt.Sprint(gfid), " ", ",", -1), "[]"), filter)
		} else {
			noofcirclemeeting = fmt.Sprintf("SELECT COUNT(tbl_poa.session_type) AS noofspoorthicircle FROM tbl_poa tbl_poa WHERE tbl_poa.type = 2 AND (tbl_poa.session_type IN (4, 5, 6, 7, 8, 9)) and tbl_poa.project_id IN (%s) and tbl_poa.user_id IN (%s) %s", strings.Trim(strings.Replace(fmt.Sprint(projectArray), " ", ",", -1), "[]"), strings.Trim(strings.Replace(fmt.Sprint(gfid), " ", ",", -1), "[]"), filter)
		}
	} else {
		noofcirclemeeting = fmt.Sprintf("SELECT COUNT(tbl_poa.session_type) AS noofspoorthicircle FROM tbl_poa tbl_poa WHERE tbl_poa.type = 2 AND (tbl_poa.session_type IN (4, 5, 6, 7, 8, 9)) and tbl_poa.date BETWEEN '%s' and '%s'", startDate, endDate)
	}
...
func GflSpoorthinewVillageCount(con *sql.DB, startDate, endDate string, projectArray []int, gflid int, filter string) int {
	...
	if len(projectArray) > 0 {
		villageQuery = fmt.Sprintf("select COUNT(DISTINCT location_id) from tbl_poa tp join training_participants t on t.project_id =tp.project_id join project p on p.id=t.project_id where t.enroll = 1 and t.project_id IN (%s) and tp.user_id IN (%s) %s", intArrayToString(projectArray), strings.Trim(strings.Replace(fmt.Sprint(gfid), " ", ",", -1), "[]"), filter)
