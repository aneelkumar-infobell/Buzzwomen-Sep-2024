...
func GelathiProgramDashboard1(w http.ResponseWriter, r *http.Request, DB *sql.DB) {
	...
	if roleid == 1 || roleid == 9 || roleid == 3 || roleid == 4 || roleid == 11 || roleid == 12 {
		...
		if len(funderListQuery) > 0 {
			...
			for rows.Next() {
				...
				if startDate != "" && endDate != "" {
					...
					getProj = fmt.Sprintf("SELECT distinct p.id from project p join tbl_poa t on t.project_id = p.id join training_participants tp on tp.tb_id=t.tb_id where tp.enroll=1 and p.funderID = %d and %s %s", funderID, dateFilter, filter)
					projResult, err := DB.Query(getProj)
					if err != nil {
						fmt.Println(err)
						return
				...
				obj["actual"] = spoorthiEnrolled(DB, startDate, endDate, projectArray, "")
				summaryActuals += obj["actual"].(int)
				obj["Gelathienrolled"] = spoorthiEnrolled(DB, startDate, endDate, projectArray, "")
		...
		if projectid > 0 {
			...
			defer rows.Close()
			if rows.Next() {
				projectList = fmt.Sprintf(`SELECT distinct p.id, projectName as name, p.startDate, p.endDate from project p join tbl_poa t on t.project_id = p.id join training_participants tp on tp.tb_id=t.tb_id where tp.enroll=1 and t.user_id = %d and %s GROUP BY t.project_id`, gfid, dateFilter)
			...
			if rows.Next() {
				if dateFilter == "" || (startDate == "" && endDate == "") {
					projectList = fmt.Sprintf(`SELECT distinct p.id, projectName as name, p.startDate, p.endDate from project p join tbl_poa t on t.project_id = p.id join training_participants tp on tp.tb_id=t.tb_id where tp.enroll=1 AND p.operations_manager = %d and %s %s GROUP BY id`, opsid, dateFilter, filter)
				} else {
					projectList = fmt.Sprintf(`SELECT distinct p.id, projectName as name, p.startDate, p.endDate from project p join tbl_poa t on t.project_id = p.id join training_participants tp on tp.tb_id=t.tb_id where tp.enroll=1 AND p.operations_manager = %d and p.startDate BETWEEN %s and %s
			...
			defer rows.Close()
			if rows.Next() {
				projectList = fmt.Sprintf(`SELECT distinct p.id, projectName as name, p.startDate, p.endDate from project p join tbl_poa t on t.project_id = p.id join training_participants tp on tp.tb_id=t.tb_id where tp.enroll=1 and operations_manager in (
					SELECT supervisorId
					from employee e
					where e.id = %d
		...
		if len(projectList) > 0 {
			...
			for rows.Next() {
				...
				prList.Actual = spoorthiEnrolled(DB, startDate, endDate, projectArray, tpFilter)
				...
				prList.Gelathienrolled = spoorthiEnrolled(DB, startDate, endDate, projectArray, "")
		...
		if rows.Next() {
			...
			for projectsList.Next() {
				...
				prList.Actual = spoorthiEnrolled(DB, startDate, endDate, projectArray, filter)
				summaryActuals += prList.Actual
				...
				summaryNoofSpoorthimodulecompleted += prList.Noofsporthicompleted
				prList.Gelathienrolled = spoorthiEnrolled(DB, startDate, endDate, projectArray, filter)
				summarySpoorthienrolled += prList.Gelathienrolled
		...
		if rows.Next() {
            ...
            where t.enroll=1 and  p.gfl_id = %d %s GROUP BY tp.project_id`, empid, filter)
			if projectid > 0 {
				getProjs = fmt.Sprintf(`Select tp.project_id as id,p.projectName as name,p.startDate, p.endDate from tbl_poa tp 
				inner join project p on p.id = tp.project_id  join training_participants t on t.tb_id= tp.tb_id
				where t.enroll=1 and p.gfl_id = %d %s GROUP BY tp.project_id`, empid, filters)
			}
			if gfid > 0 {
				getProjs = fmt.Sprintf(`Select tp.project_id as id,p.projectName as name,p.startDate, p.endDate from tbl_poa tp 
				inner join project p on p.id = tp.project_id  join training_participants t on t.tb_id= tp.tb_id
