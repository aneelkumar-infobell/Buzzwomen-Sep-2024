...
func GetTrainersList(w http.ResponseWriter, r *http.Request, DB *sql.DB) {
	...
	fields := "id, CONCAT(first_name, ' ', last_name)"
	query := fmt.Sprintf("SELECT %s FROM employee WHERE status = 1 AND empRole = 5 ORDER BY id DESC", fields)
