...
func SetGreenMotivator(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	// declaring req variable to refer fields of GreenRequest struct.
	var req GreenRequest
	// declarig res variable to refer fields of GreenResponse struct.
	var res GreenResponse

	// dumping the body parameters into GreenRequest struct.
	...
	checkIfEnrolledPreviously := "SELECT id FROM training_participants WHERE GreenMotivators = 1 AND id = ? AND tb_id = ?"
	...
	if participantID > 0 {
		updateSet = "UPDATE training_participants SET GreenMotivators = '0', gelathi_id = ?, GreenMotivatorsDate = ?, GreenMotivatorsEnrolledProject = ? WHERE tb_id = ? AND id = ?"
	} else {
		updateSet = "UPDATE training_participants SET GreenMotivators = '1', gelathi_id = ?, GreenMotivatorsDate = ?, GreenMotivatorsEnrolledProject = ? WHERE tb_id = ? AND id = ?"
	}
	//executing query.
	...
	res.Code = 200
	res.Success = true
	res.Message = "Green Enrollment Updated Successfully."
